    ...
    public ResponseEntity<BaseResponse> openAccount(@RequestBody OpenAccountCommand command) {
        ...
        try {
            ...
            return new ResponseEntity<>(new
                    OpenAccountResponse("Bank Account open request completed successfully", id),
                    HttpStatus.CREATED);
            ...
            var safeErrorMessage = MessageFormat.format("Error while processing request to open a new " +
                            "bank account for id - {0}",
                    id);
